...
public abstract class UIBasicSprite : UIWidget
{
	...
	override public int minWidth
	{
		get
		{
            if (type == Type.Sliced || type == Type.Advanced || type == Type.Mirrored || type == Type.Quarter)
            {
...
#region Fill Functions
    ...
    void MirroredFill(List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
    {
        Vector4 br = border * pixelSize;
        ...
        {
            mTempPos[0].x = (direction == 1) ? v.x : v.z;
            mTempPos[0].y = v.y;
            mTempPos[3].x = (v.x + v.z) / 2;
            mTempPos[3].y = v.w;

            if (mFlip == Flip.Horizontally || mFlip == Flip.Both)
            {
                mTempPos[1].x = mTempPos[0].x + br.z * direction;
                mTempPos[2].x = mTempPos[3].x - br.x * direction;
            ...
            {
                mTempPos[1].x = mTempPos[0].x + br.x * direction;
                mTempPos[2].x = mTempPos[3].x - br.z * direction;
    ...
    void QuarterFill(List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
    {
        ...
        // 1 = Left, -1 = Right
        for (int directionH = 1; directionH >= -1; directionH -= 2)
        {
            // 1 = Bottom, -1 = Top
            for (int directionV = 1; directionV >= -1; directionV -= 2)
            {
                mTempPos[0].x = (directionH == 1) ? v.x : v.z;
                mTempPos[0].y = (v.w + v.y) / 2;
                mTempPos[3].x = (v.x + v.z) / 2;
                mTempPos[3].y = (directionV == 1) ? v.w : v.y;

                if (mFlip == Flip.Horizontally || mFlip == Flip.Both)
                {
                    mTempPos[1].x = mTempPos[0].x + br.z * directionH;
                    mTempPos[2].x = mTempPos[3].x - br.x * directionH;
                ...
                {
                    mTempPos[1].x = mTempPos[0].x + br.x * directionH;
                    mTempPos[2].x = mTempPos[3].x - br.z * directionH;
                ...
                if (mFlip == Flip.Vertically || mFlip == Flip.Both)
                {
                    mTempPos[1].y = mTempPos[0].y + br.w * directionV;
                    mTempPos[2].y = mTempPos[3].y - br.y * directionV;
                ...
                {
                    mTempPos[1].y = mTempPos[0].y + br.y * directionV;
                    mTempPos[2].y = mTempPos[3].y - br.w * directionV;
